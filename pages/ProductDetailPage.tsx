...
const ProductDetailPage: React.FC = () => {
  ...
  const { addToCart, cart, getReviewsForProduct, isAuthenticated, checkIfUserPurchasedProduct, user } = useAppContext();
  ...
  const fetchProductAndReviews = useCallback(async () => {
      if (!productId) return;
      try {
          // Fetch both product and reviews concurrently
          ...
          if (productData) {
              // Manually calculate ratings from reviews, as the backend API for products doesn't provide them
              const reviewCount = reviewsData.length;
              const averageRating = reviewCount > 0
                  ? reviewsData.reduce((acc, review) => acc + review.rating, 0) / reviewCount
                  : 0;

              // Combine API product data with calculated review data
              setProduct({
                  ...productData,
                  averageRating,
                  reviewCount,
          ...
          setProduct(null);
          setReviews([]); // Also clear reviews on error
      } finally {
          // This will be set on initial load because of the useEffect below
          setLoading(false);
      }
  }, [productId, getReviewsForProduct]);

  useEffect(() => {
    setLoading(true); // Set loading only for the initial mount/productId change
    ...
    <>
      ...
      <div id="reviews" className="mt-16">
        ...
        {userCanReview && !hasUserReviewed && (
            <ReviewForm productId={product.id} onSubmit={fetchProductAndReviews} />
        )}
        {hasUserReviewed && (
