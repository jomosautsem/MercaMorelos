...
const HomePage: React.FC = () => {
  ...
  return (
    <>
      ...
      {/* Gradient section for Category Carousel */}
      <div className="bg-gradient-to-b from-primary via-amber-200 to-background">
        <CategoryCarousel />
      </div>

      {/* Dark Section with Featured Products and Secondary Carousel */}
      <div className="bg-gradient-to-b from-slate-900 to-slate-800">
        {/* Featured Products Section */}
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <h2 className="text-4xl sm:text-5xl lg:text-6xl font-black mb-12 text-center text-primary">
          ...
          {loading ? (
            <div className="text-center py-10">
               <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                  {Array.from({ length: 8 }).map((_, index) => (
                      <div key={index} className="bg-slate-700 rounded-lg shadow-md h-96 animate-pulse"></div>
                  ))}
               </div>
            </div>
          ) : error ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
             ...
             <div className="text-center py-10 text-slate-400">No hay productos destacados en este momento.</div>
          )}
        </div>
