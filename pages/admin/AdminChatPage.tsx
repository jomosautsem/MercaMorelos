...
const AdminChatPage: React.FC = () => {
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-10rem)] bg-surface rounded-lg shadow-md">
            ...
            <div ref={chatContainerRef} className="flex-1 p-4 overflow-y-auto space-y-4">
                {conversation.map(msg => (
                    <div key={msg.id} className={`flex ${msg.from === 'admin' ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-xs lg:max-w-md px-4 py-2 rounded-lg ${msg.from === 'admin' ? 'bg-primary text-white' : 'bg-gray-200 text-on-surface'}`}>
                            <p>{msg.text}</p>
